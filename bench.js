...
const shouldWrite = Boolean(process.env.WRITE);

async function exec(year, day) {
  const output = [];

  day = `${day}`.padStart(2, '0');
  const data = await promises.readFile(`./data/${year}-${day}.txt`, 'utf8');
  const parts = await Promise.all([
    import(`./src/${year}/day-${day}/part-1.js`).catch(() => ({
      default: null,
    })),
    import(`./src/${year}/day-${day}/part-2.js`).catch(() => ({
      default: null,
    })),
  ]);

  for (const [idx, { default: fn }] of parts.entries()) {
    ...
    process.stdout.write(
      `[${year}] Day ${day} Part ${idx + 1} - ${ms(
        timeTotal / config.runs / 1e6,
        {
    ...
    output.push(
      ms(timeTotal / config.runs / 1e6, { formatSubMilliseconds: true })
    );
  }
...
if (config.day !== null) {
  ...
  const days = (
    await promises.readdir(`./src/${config.year}`, { withFileTypes: true })
  )
  ...
  if (shouldWrite) {
    let readmeContents = await promises.readFile(
      `./src/${config.year}/README.md`,
      'utf8'
    );
    const r = /<!-- BENCH TABLE -->([\s\S]*?)<!-- \/BENCH TABLE -->/g;
